    ...
    for attempt in range(retries):
        try:
            response = requests.get(
                'https://api.themoviedb.org/3/movie/{}?api_key=bcd74fe8125bdfd9c6cb2559ffb8c66c'.format(movie_id))
            data = response.json()
            try:
...
def fetch_person_details(id_):
    ...
    for attempt in range(retries):
        try:
            response = requests.get(
                f'https://api.themoviedb.org/3/person/{id_}?api_key=bcd74fe8125bdfd9c6cb2559ffb8c66c')
            data = response.json()
            try:
                url = f'https://image.tmdb.org/t/p/w220_and_h330_face{data["profile_path"]}'
                biography = data['biography'] if data['biography'] else "No biography available"
                return url, biography
            except KeyError:
                return "https://media.istockphoto.com/vectors/error-icon-vector-illustration-vector-id922024224?k=6&m=922024224&s=612x612&w=0&h=LXl8Ul7bria6auAXKIjlvb6hRHkAodTqyqBeA6K7R54=", "No biography available"
        except requests.exceptions.RequestException as e:
            print(f"Error fetching details for person {id_}: {e}")
            time.sleep(2)  # Wait before retrying
    return "https://media.istockphoto.com/vectors/error-icon-vector-illustration-vector-id922024224?k=6&m=922024224&s=612x612&w=0&h=LXl8Ul7bria6auAXKIjlvb6hRHkAodTqyqBeA6K7R54=", "No biography available"
